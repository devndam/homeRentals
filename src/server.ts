...
async function bootstrap() {
  try {
    ...
    cron.schedule('0 8 * * *', async () => {
      logger.info('Cron', 'Running daily rent jobs...');
      try {
        ...
        logger.info('Cron', `Renewal invoices: ${renewalResult.created} created.`);
        ...
        logger.info('Cron', `Rent reminders: ${reminderResult.sent} email(s) sent.`);
        ...
        logger.info('Cron', `Stale bookings cancelled: ${cleanupResult.cancelled}`);
      } catch (err) {
        logger.error('Cron', err);
      }
    });
    logger.info('Cron', 'Daily rent job scheduled (8:00 AM)');

    // Start server
    app.listen(env.port, () => {
      logger.info('Server', `Running on port ${env.port}`);
      logger.info('Server', `API prefix: ${env.apiPrefix}`);
